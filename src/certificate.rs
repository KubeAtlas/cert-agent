...
use crate::redis_client::{RedisClient, CertificateRecord};
...
impl CertificateManager {
    ...
    pub async fn renew_certificate(&self, certificate_id: &str, validity_days: Option<u32>) -> Result<IssuedCertificate> {
        // Get existing certificate record
        let cert_record = self.redis.get_certificate(certificate_id).await?
            .ok_or_else(|| CertAgentError::CertificateNotFound(certificate_id.to_string()))?;
        
        if cert_record.status != "active" {
            return Err(CertAgentError::Certificate(format!("Cannot renew certificate with status: {}", cert_record.status)));
        }
        ...
        self.redis.update_certificate_status(certificate_id, "revoked").await?;
        self.redis.publish_event("revoked", certificate_id).await?;
        
        // Publish renewal event
        self.redis.publish_event("renewed", &new_cert.certificate_id).await?;
        
        Ok(new_cert)
    }
    
    pub async fn revoke_certificate(&self, certificate_id: &str, reason: Option<&str>) -> Result<()> {
    ...
    pub async fn get_certificate_status(&self, certificate_id: &str) -> Result<Option<CertificateRecord>> {
    ...
    pub async fn get_expiring_certificates(&self) -> Result<Vec<CertificateRecord>> {
        self.redis.get_expiring_certificates(self.config.renewal_threshold_days).await
    }
}
