...
impl CertificateWatcher {
    ...
    pub async fn start(&self) -> Result<()> {
        info!("Starting certificate watcher with {} second intervals", self.config.check_interval_seconds);
        ...
        loop {
            ...
            if let Err(e) = self.check_and_renew_certificates(renewal_semaphore.clone()).await {
    ...
    async fn check_and_renew_certificates(&self, renewal_semaphore: Arc<Semaphore>) -> Result<()> {
        ...
        info!("Found {} certificates that need renewal", expiring_certs.len());
        ...
        for cert_record in expiring_certs {
            ...
            let task = tokio::spawn(async move {
                ...
                match cert_manager.renew_certificate(&cert_id, None).await {
                    Ok(new_cert) => {
                        info!("Successfully renewed certificate: {} -> {}", cert_id, new_cert.certificate_id);
                        
                        // Publish renewal event
                        if let Err(e) = redis.publish_event("auto_renewed", &new_cert.certificate_id).await {
        ...
        info!("Certificate renewal batch completed: {} successful, {} failed", 
    ...
    pub async fn check_certificate_health(&self) -> Result<()> {
        ...
        info!("Certificate health check: {} active, {} expired, {} revoked", 
              active_count, expired_count, revoked_count);
        
        // Publish health metrics
        let health_data = format!("active:{},expired:{},revoked:{}", 
                                 active_count, expired_count, revoked_count);
        self.redis.publish_event("health_check", &health_data).await?;
    ...
    pub async fn cleanup_expired_certificates(&self, days_old: u32) -> Result<()> {
        ...
        for cert in all_certs {
            if cert.expires_at < cutoff_time {
                ...
                if let Err(e) = self.redis.delete_certificate(&cert.certificate_id).await {
                    warn!("Failed to delete expired certificate {}: {}", cert.certificate_id, e);
                } else {
                    cleaned_count += 1;
        ...
        if cleaned_count > 0 {
            info!("Cleaned up {} expired certificates older than {} days", cleaned_count, days_old);
            self.redis.publish_event("cleanup", &format!("removed:{}", cleaned_count)).await?;
        }
